...
public class PixyI2C{
	...
	*/
	//This method parses pixy data, and 
	public PixyPacket readPacket(int Signature, PrintWriter printWriter) throws PixyException { //The signature should be which number object in 
		int Checksum;	
		String convertByte; //pixymon you are trying to get data for
		int Sig;
		...
		printWriter.print("raw Data: ");
		for (int i = 0; i <= 16; i++) {
			convertByte = String.format(HEX_FORMAT, rawData[i]);
			printWriter.print(convertByte + ", ");
			SmartDashboard.putString("pixyData[" + i + "]", convertByte );
			convertByte = String.format(HEX_FORMAT, rawData[i+1]);
			printWriter.print(convertByte + ", ");
